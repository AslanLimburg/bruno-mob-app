...
// Конфигурация программ (perLevel = сумма ПОСЛЕ вычета gas fee)
const PROGRAMS = {
  'GS-I': { 
    price: 5, 
    levels: 4, 
    perLevel: 0.88,  // Реферу после вычета gas fee
    gasFee: 0.02
  },
  'GS-II': { 
    price: 50, 
...
const joinProgram = async (req, res) => {
  try {
    ...
    await transaction(async (client) => {
      ...
      console.log(`💎 Distribution for ${program}:`);
      console.log(`   Price: ${config.price} BRT`);
      console.log(`   Per level: ${config.perLevel} BRT (to referrer) + ${config.gasFee} BRT (gas fee)`);

      // Распределить по ВСЕМ уровням
      for (let i = 0; i < config.levels; i++) {
        const recipientId = hierarchy[i];
        const referrerAmount = config.perLevel;  // Сумма рефереру (уже без gas fee)
        ...
        totalGasFees += config.gasFee;

        // Если есть реферал
        if (recipientId && recipientId !== adminId) {
          await client.query(
            'UPDATE user_balances SET balance = balance + $1 WHERE user_id = $2 AND crypto = $3',
            [referrerAmount, recipientId, 'BRT']
          );

          await client.query(
            'INSERT INTO transactions (from_user_id, to_user_id, crypto, amount, type, status, metadata) VALUES ($1, $2, $3, $4, $5, $6, $7)',
            [userId, recipientId, 'BRT', referrerAmount, 'club_avalanche_commission', 'completed', JSON.stringify({program, level: i + 1})]
          );

          totalToReferrals += referrerAmount;
          console.log(`   ✅ Level ${i + 1}: ${referrerAmount.toFixed(2)} BRT → user ${recipientId}, gas: ${config.gasFee} BRT`);
        } 
        // Если реферала нет → на admin
        else {
          await client.query(
            'UPDATE user_balances SET balance = balance + $1 WHERE user_id = $2 AND crypto = $3',
            [referrerAmount, adminId, 'BRT']
          );

          await client.query(
            'INSERT INTO transactions (from_user_id, to_user_id, crypto, amount, type, status, metadata) VALUES ($1, $2, $3, $4, $5, $6, $7)',
            [userId, adminId, 'BRT', referrerAmount, 'club_avalanche_no_referrer', 'completed', JSON.stringify({program, level: i + 1})]
          );

          console.log(`   ✅ Level ${i + 1}: ${referrerAmount.toFixed(2)} BRT → admin (no referrer), gas: ${config.gasFee} BRT`);
        }

        totalPaid += (referrerAmount + config.gasFee);
      }

      // Остаток идет на admin
      const remainder = config.price - totalPaid;
      
      if (remainder > 0.001) {
        await client.query(
          'UPDATE user_balances SET balance = balance + $1 WHERE user_id = $2 AND crypto = $3',
          [remainder, adminId, 'BRT']
        );

        await client.query(
          'INSERT INTO transactions (from_user_id, to_user_id, crypto, amount, type, status, metadata) VALUES ($1, $2, $3, $4, $5, $6, $7)',
          [userId, adminId, 'BRT', remainder, 'club_avalanche_remainder', 'completed', JSON.stringify({program})]
        );

        console.log(`   ✅ Remainder: ${remainder.toFixed(2)} BRT → admin`);
        totalPaid += remainder;
      }

      console.log(`🎉 Distribution complete:`);
      console.log(`   Total: ${totalPaid.toFixed(2)} BRT of ${config.price} BRT`);
      console.log(`   - To referrals: ${totalToReferrals.toFixed(2)} BRT`);
      console.log(`   - Gas fees: ${totalGasFees.toFixed(2)} BRT`);
      console.log(`   - To admin: ${(totalPaid - totalToReferrals - totalGasFees).toFixed(2)} BRT`);
