...
const Dashboard = ({addNotification}) => {
  ...
  const totalBalance = user?.balances?.BRT || 5000;
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        ...
        <div className="tabs">
          ...
          <button className={activeTab==="referrals"?"tab active":"tab"} onClick={()=>setActiveTab("referrals")}>Referrals</button>
        </div>
        ...
        {activeTab==="lottery" && (
          <div className="lottery-section">
            <h3>🎲 Bruno Token Lottery</h3>
            <div className="lottery-jackpot">
              <div className="jackpot-label">Current Jackpot</div>
              <div className="jackpot-amount">75,000 BRT</div>
              <div className="jackpot-usd">≈ $7,500</div>
            </div>
            
            <div className="lottery-buy-section">
              <h4>Buy Lottery Tickets</h4>
              <p className="lottery-info">10 BRT per ticket • Draw every Sunday at 8 PM UTC</p>
              <div className="ticket-selector">
                <button onClick={()=>setTicketCount(Math.max(1,ticketCount-1))} className="btn-ticket-change">−</button>
                <input 
                  type="number" 
                  value={ticketCount} 
                  ...
                  min="1"
                />
                <button onClick={()=>setTicketCount(ticketCount+1)} className="btn-ticket-change">+</button>
              </div>
              <button onClick={handleBuyTickets} className="btn-buy-tickets">
                Buy {ticketCount} Ticket{ticketCount>1?"s":""} for {ticketCount*10} BRT
              </button>
            </div>
            
            <h4>Recent Draws</h4>
            <div className="lottery-history">
              {mockLottery.map(draw=>(
                <div key={draw.id} className="lottery-draw-item">
                  <div className="draw-info">
                    <div className="draw-name">{draw.draw}</div>
                    <div className="draw-date">{draw.date}</div>
                  </div>
                  <div className="draw-jackpot">{draw.jackpot}</div>
                  <div className="draw-winner">Winner: {draw.winner}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="referrals-section">
            ...
            <div className="referral-card">
              ...
              <div className="referral-code-section">
                <div className="referral-label">Referral Code</div>
                <div className="referral-code-display">
                  <code>{referralCode}</code>
                  <button 
                    onClick={()=>{navigator.clipboard.writeText(referralCode);addNotification("success","Code copied!")}} 
              ...
              <div className="referral-link-section">
                ...
                <div className="referral-code-display">
                  <code className="referral-link">https://brunotoken.com/ref/{referralCode}</code>
                  <button 
                    onClick={()=>{navigator.clipboard.writeText(`https://brunotoken.com/ref/${referralCode}`);addNotification("success","Link copied!")}} 
                    className="btn-copy-small"
                  >
                    📋
                  </button>
              ...
              <p className="referral-info">
                Earn 10% commission on every transaction your referrals make! Share your code with friends.
              </p>
            </div>
            ...
            <div className="referral-stats-grid">
              <div className="stat-card">
                ...
                <div className="stat-value">{mockReferrals.length}</div>
              ...
              <div className="stat-card">
                ...
                <div className="stat-label">Total Earned (BRT)</div>
              </div>
              
              <div className="stat-card">
                <div className="stat-icon">⚡</div>
                <div className="stat-value">{mockReferrals.filter(r=>r.status==="active").length}</div>
            ...
            <div className="referrals-list-section">
              ...
              <div className="transactions-list">
                {mockReferrals.map(ref => (
                  <div key={ref.id} className="transaction-item">
                    <div className="tx-icon" style={{background: "#4CAF50"}}>👤</div>
                    <div className="tx-details">
                      <div className="tx-main">
                        <span className="tx-type">{ref.name}</span>
                      </div>
                      <div className="tx-meta">
                        <span className="tx-date">Joined: {ref.joined}</span>
                        <span className="tx-address">{ref.email}</span>
                      </div>
                    </div>
                    <div className="tx-amount-status">
                      <span className="tx-amount">+{ref.earnings}</span>
                      <span className="tx-status" style={{color: "#4CAF50"}}>{ref.status}</span>
                    </div>
                  </div>
                ))}
              </div>
